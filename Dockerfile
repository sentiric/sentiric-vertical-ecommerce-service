...
RUN CGO_ENABLED=0 GOOS=linux go build \
    ...
    -o /app/bin/sentiric-vertical-ecommerce-service ./cmd/vertical-ecommerce-service
...
COPY --from=builder /app/bin/sentiric-vertical-ecommerce-service .
RUN chown appuser:appgroup ./sentiric-vertical-ecommerce-service

# GÜVENLİK: Kullanıcıyı değiştir
USER appuser

ENTRYPOINT ["./sentiric-vertical-ecommerce-service"]