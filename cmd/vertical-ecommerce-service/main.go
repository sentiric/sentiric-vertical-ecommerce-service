// sentiric-vertical-ecommerce-service/cmd/vertical-ecommerce-service/main.go
package main
...
import (
	...
	"github.com/sentiric/sentiric-vertical-ecommerce-service/internal/config"
	"github.com/sentiric/sentiric-vertical-ecommerce-service/internal/logger"
	"github.com/sentiric/sentiric-vertical-ecommerce-service/internal/server"
...
const serviceName = "vertical-ecommerce-service"
...
func main() {
	...
	log.Info().
		...
		Str("profile", cfg.Env).
		Msg("🚀 Sentiric Vertical Ecommerce Service başlatılıyor...")
	...
	verticalv1.RegisterEcommerceServiceServer(grpcServer, &ecommerceHandler{})
...
type ecommerceHandler struct {
	verticalv1.UnimplementedEcommerceServiceServer
}

func (*ecommerceHandler) GetOrderStatus(ctx context.Context, req *verticalv1.GetOrderStatusRequest) (*verticalv1.GetOrderStatusResponse, error) {
	log := zerolog.Ctx(ctx).With().Str("rpc", "GetOrderStatus").Str("order_id", req.GetOrderId()).Logger()
	log.Info().Msg("GetOrderStatus isteği alındı (Placeholder)")

	// Simüle edilmiş sipariş durumu
	return &verticalv1.GetOrderStatusResponse{
		Status: "Shipped",
	}, nil
}
